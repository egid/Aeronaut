...
// -----------------------------------------------------
// Getter for 'DARK_THEME'
bool enamel_get_DARK_THEME(){
	Tuple* tuple = dict_find(&s_dict, 155210400);
	return tuple ? tuple->value->int32 == 1 : true;
}
// -----------------------------------------------------

// -----------------------------------------------------
// Getter for 'COLOR_GMT_HAND'
GColor enamel_get_COLOR_GMT_HAND(){
	Tuple* tuple = dict_find(&s_dict, 538346706);
	return tuple ? GColorFromHEX(tuple->value->int32) : GColorFromHEX(0xFFAA00);
}
// -----------------------------------------------------

// -----------------------------------------------------
// Getter for 'OFFSET_GMT_HAND'
int32_t enamel_get_OFFSET_GMT_HAND(){
	Tuple* tuple = dict_find(&s_dict, 2518972927);
...
// -----------------------------------------------------
// Getter for 'SHOW_SECONDS'
bool enamel_get_SHOW_SECONDS(){
	Tuple* tuple = dict_find(&s_dict, 4113260241);
	return tuple ? tuple->value->int32 == 1 : false;
}
// -----------------------------------------------------

// -----------------------------------------------------
// Getter for 'COLOR_SECOND_HAND'
GColor enamel_get_COLOR_SECOND_HAND(){
	Tuple* tuple = dict_find(&s_dict, 1376507077);
...
// -----------------------------------------------------
// Getter for 'SLEEP_MODE_ENABLED'
bool enamel_get_SLEEP_MODE_ENABLED(){
	Tuple* tuple = dict_find(&s_dict, 291440801);
	return tuple ? tuple->value->int32 == 1 : false;
}
// -----------------------------------------------------
...
static uint32_t prv_map_messagekey(const uint32_t key){
	if( key == MESSAGE_KEY_DARK_THEME) return 155210400;
	if( key == MESSAGE_KEY_COLOR_GMT_HAND) return 538346706;
	if( key == MESSAGE_KEY_OFFSET_GMT_HAND) return 2518972927;
	if( key == MESSAGE_KEY_SHOW_SECONDS) return 4113260241;
	if( key == MESSAGE_KEY_COLOR_SECOND_HAND) return 1376507077;
#if defined(PBL_HEALTH)
	if( key == MESSAGE_KEY_SLEEP_MODE_ENABLED) return 291440801;
#endif
	return 0;
}
...
static bool prv_is_setting_message(const DictionaryIterator *iter){
	if( dict_find(iter, MESSAGE_KEY_DARK_THEME) ) return true;
	if( dict_find(iter, MESSAGE_KEY_COLOR_GMT_HAND) ) return true;
	if( dict_find(iter, MESSAGE_KEY_OFFSET_GMT_HAND) ) return true;
	if( dict_find(iter, MESSAGE_KEY_SHOW_SECONDS) ) return true;
	if( dict_find(iter, MESSAGE_KEY_COLOR_SECOND_HAND) ) return true;
#if defined(PBL_HEALTH)
	if( dict_find(iter, MESSAGE_KEY_SLEEP_MODE_ENABLED) ) return true;
#endif
	return false;	
}
